...
public class WorldGenState extends PersistentState {
    public static Map<DimensionType, PersistentStateManager> persistentStateManagerMap = new HashMap<>();
    private static final Logger LOGGER = LogManager.getFormatterLogger("rer-wgs");
    public boolean playerDirty = false;

    public static void registerPsm(PersistentStateManager psm, DimensionType dt) {
        if (persistentStateManagerMap.containsKey(dt)) {
            LOGGER.warn("Registering psm %s for already known dt %s?", psm, dt);
        }
        persistentStateManagerMap.put(dt, psm);
    }
    ...
    public static WorldGenState byDimension(DimensionType dim) {
        ...
        PersistentStateManager psm = persistentStateManagerMap.get(dim);
        return psm.getOrCreate(() -> new WorldGenState(name, dim), name);
    }
    ...
    public WorldGenState(String string_1, DimensionType type) {
        super(string_1);
    }

    public void sendToPlayer(ServerPlayerEntity player, CompoundTag tag, DimensionType type) {
        ServerSidePacketRegistry.INSTANCE.sendToPlayer(player, RoughlyEnoughResources.SEND_WORLD_GEN_STATE, new PacketByteBuf(Unpooled.buffer()).writeIdentifier(Registry.DIMENSION_TYPE.getId(type)).writeCompoundTag(tag));
    }
