...
public class LootOutput {
    ...
    public void addExtraText(String text) {
        if (nowInverted)
            text = I18n.translate("rer.condition.invert", text);
        if (extraText != null) {
            if (lastInverted && nowInverted)
                extraText = I18n.translate("rer.condition.or", extraText, text);
            else
                extraText = I18n.translate("rer.function.and", extraText, text);
        } else {
            extraText = text;
        }
    ...
    public void addExtraTextCount(String text) {
        if (extraTextCount != null) {
            extraTextCount = I18n.translate("rer.function.and", extraTextCount, text);
        } else {
            extraTextCount = text;
        }
    }
